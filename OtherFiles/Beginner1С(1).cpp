...
void SkaterMove (int *xSkater, int *ySkater);
...
void SkateMove (int* xSkate, int* ySkate)
    {
    if (GetAsyncKeyState ('K')) *xSkate = *xSkate + 5;
    if (GetAsyncKeyState ('H'))  *xSkate = *xSkate - 5;
    if (GetAsyncKeyState ('U'))    *ySkate = *ySkate + 5;
    if (GetAsyncKeyState ('J'))  *ySkate = *ySkate - 5;
    }

void SkaterMove (int* xSkater, int* ySkater)
    {
    if (GetAsyncKeyState (VK_RIGHT)) *xSkater = *xSkater + 5;
    if (GetAsyncKeyState (VK_LEFT))  *xSkater = *xSkater - 5;
    if (GetAsyncKeyState (VK_UP))    *ySkater = *ySkater + 5;
    if (GetAsyncKeyState (VK_DOWN))  *ySkater = *ySkater - 5;
